...
<div class="row ">
  <div class="col-xl-12">
    <div class="card">
      <div class="card-body">
        <h4 class="page-title">
          <i class="mdi mdi-library title_icon"></i> <?php echo get_phrase('mean_issue_deadline'); ?>
        </h4>
      </div> <!-- end card body-->
    </div> <!-- end card -->
  </div><!-- end col-->
      ...
      <div class="card-body">
        <div class="row mb-3">
          <div class="col-md-1 mb-1"></div>
            ...
            <div class="col-md-2">
                <button class="btn btn-block btn-secondary" onclick="filter_mean_deadline()" ><?php echo get_phrase('filter'); ?></button>
            </div>
          </div>
...
function filter_mean_deadline(){
    var mean_id = $('#mean_id').val();
    var date = $('#date').val();
    var tgl = $('#tgl').val();
    $.ajax({
        type : 'post',
        url: '<?php echo route('mean_issue/filter_deadline/') ?>',
...
var showAllMeanDeadline = function () {
  var url = '<?php echo route('mean_issue/list_deadline'); ?>';
  $.ajax({
    type : 'GET',
    url: url,
    success : function(response) {
