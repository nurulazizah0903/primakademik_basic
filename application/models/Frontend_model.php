...
class Frontend_model extends CI_Model {
  ...
  function event_create() {
    ...
    $history_data['ket'] = 'Mengisi data frontend event';
  ...
  function add_news() {
    ...
    $history_data['ket'] = 'Mengisi data frontend news';
    $history_data['id_user'] = $this->session->set_userdata('user_id');
    $this->db->insert('history', $history_data);
  }
  ...
  function add_frontend_gallery() {
    ...
    $history_data['ket'] = 'Mengisi data frontend gallery';
    $history_data['id_user'] = $this->session->set_userdata('user_id');
    $this->db->insert('history', $history_data);
  ...
  public function upload_gallery_photo($gallery_id) {
    if (isset($_FILES['gallery_photo']) && !empty($_FILES['gallery_photo']['name'])) {
      ...
      $history_data['ket'] = 'Mengisi data frontend gallery image';
      $history_data['id_user'] = $this->session->set_userdata('user_id');
      $this->db->insert('history', $history_data);
  ...
  function maps_update() {
    ...
    $response = array(
      'status' => true,
      'notification' => get_phrase('maps_updated')
    );
    return json_encode($response);
  }
  ...
  function superiority_update() {
    $data['superiority_title_one'] = $this->input->post('superiority_title_one');
    $data['superiority_description_one'] = $this->input->post('superiority_description_one');

    $data['superiority_title_two'] = $this->input->post('superiority_title_two');
    $data['superiority_description_two'] = $this->input->post('superiority_description_two');

    $data['superiority_title_tree'] = $this->input->post('superiority_title_tree');
    $data['superiority_description_tree'] = $this->input->post('superiority_description_tree');

    $data['superiority_title_four'] = $this->input->post('superiority_title_four');
    $data['superiority_description_four'] = $this->input->post('superiority_description_four');

    $this->db->where('id', 1);
    $this->db->update('frontend_settings', $data);

    if ($_FILES['superiority_image_one']['name'] != "") {
      move_uploaded_file($_FILES['superiority_image_one']['tmp_name'], 'uploads/system/superiority/superiority_image_one.jpg');
    }

    if ($_FILES['superiority_image_two']['name'] != "") {
      move_uploaded_file($_FILES['superiority_image_two']['tmp_name'], 'uploads/system/superiority/superiority_image_two.jpg');
    }

    if ($_FILES['superiority_image_tree']['name'] != "") {
      move_uploaded_file($_FILES['superiority_image_tree']['tmp_name'], 'uploads/system/superiority/superiority_image_tree.jpg');
    }

    if ($_FILES['superiority_image_four']['name'] != "") {
      move_uploaded_file($_FILES['superiority_image_four']['tmp_name'], 'uploads/system/superiority/superiority_image_four.jpg');
    }

    $response = array(
			'status' => true,
			'notification' => get_phrase('superiority_updated')
		);
		return json_encode($response);
  }

  function new_flash_update() {
    $data['new_flash_title_one'] = $this->input->post('new_flash_title_one');
    $data['new_flash_description_one'] = $this->input->post('new_flash_description_one');

    $data['new_flash_title_two'] = $this->input->post('new_flash_title_two');
    $data['new_flash_description_two'] = $this->input->post('new_flash_description_two');

    $data['new_flash_title_tree'] = $this->input->post('new_flash_title_tree');
    $data['new_flash_description_tree'] = $this->input->post('new_flash_description_tree');

    $this->db->where('id', 1);
    $this->db->update('frontend_settings', $data);

    if ($_FILES['new_flash_image_one']['name'] != "") {
      move_uploaded_file($_FILES['new_flash_image_one']['tmp_name'], 'uploads/system/new_flash/new_flash_image_one.jpg');
    }

    if ($_FILES['new_flash_image_two']['name'] != "") {
      move_uploaded_file($_FILES['new_flash_image_two']['tmp_name'], 'uploads/system/new_flash/new_flash_image_two.jpg');
    }

    if ($_FILES['new_flash_image_tree']['name'] != "") {
      move_uploaded_file($_FILES['new_flash_image_tree']['tmp_name'], 'uploads/system/new_flash/new_flash_image_tree.jpg');
    }


    $response = array(
			'status' => true,
			'notification' => get_phrase('new_flash_update')
		);
		return json_encode($response);
  }
  ...
  function gallery_update() {
    ...
    if ($_FILES['gallery_image_one']['name'] != "") {
      move_uploaded_file($_FILES['gallery_image_one']['tmp_name'], 'uploads/system/gallery/gallery_image_one.jpg');
    }

    if ($_FILES['gallery_image_two']['name'] != "") {
      move_uploaded_file($_FILES['gallery_image_two']['tmp_name'], 'uploads/system/gallery/gallery_image_two.jpg');
    }

    if ($_FILES['gallery_image_tree']['name'] != "") {
      move_uploaded_file($_FILES['gallery_image_tree']['tmp_name'], 'uploads/system/gallery/gallery_image_tree.jpg');
    }

    if ($_FILES['gallery_image_4']['name'] != "") {
      move_uploaded_file($_FILES['gallery_image_4']['tmp_name'], 'uploads/system/gallery/gallery_image_4.jpg');
    }

    if ($_FILES['gallery_image_5']['name'] != "") {
      move_uploaded_file($_FILES['gallery_image_5']['tmp_name'], 'uploads/system/gallery/gallery_image_5.jpg');
    }

    if ($_FILES['gallery_image_6']['name'] != "") {
      move_uploaded_file($_FILES['gallery_image_6']['tmp_name'], 'uploads/system/gallery/gallery_image_6.jpg');
    }

    if ($_FILES['gallery_image_7']['name'] != "") {
      move_uploaded_file($_FILES['gallery_image_7']['tmp_name'], 'uploads/system/gallery/gallery_image_7.jpg');
    }

    if ($_FILES['gallery_image_8']['name'] != "") {
      move_uploaded_file($_FILES['gallery_image_8']['tmp_name'], 'uploads/system/gallery/gallery_image_8.jpg');
    }

    if ($_FILES['gallery_image_9']['name'] != "") {
      move_uploaded_file($_FILES['gallery_image_9']['tmp_name'], 'uploads/system/gallery/gallery_image_9.jpg');
    }

    if ($_FILES['gallery_image_10']['name'] != "") {
      move_uploaded_file($_FILES['gallery_image_10']['tmp_name'], 'uploads/system/gallery/gallery_image_10.jpg');
    }
  ...
  public function get_superiority_image_one() {
    return base_url('uploads/system/superiority/superiority_image_one.jpg');
  }

  public function get_superiority_image_two() {
    return base_url('uploads/system/superiority/superiority_image_two.jpg');
  }

  public function get_superiority_image_tree() {
    return base_url('uploads/system/superiority/superiority_image_tree.jpg');
  }

  public function get_superiority_image_four() {
    return base_url('uploads/system/superiority/superiority_image_four.jpg');
  }

  public function get_new_flash_image_one() {
    return base_url('uploads/system/new_flash/new_flash_image_one.jpg');
  }

  public function get_new_flash_image_two() {
    return base_url('uploads/system/new_flash/new_flash_image_two.jpg');
  }

  public function get_new_flash_image_tree() {
    return base_url('uploads/system/new_flash/new_flash_image_tree.jpg');
  }

  public function get_gallery_image_one() {
    return base_url('uploads/system/gallery/gallery_image_one.jpg');
  }

  public function get_gallery_image_two() {
    return base_url('uploads/system/gallery/gallery_image_two.jpg');
  }

  public function get_gallery_image_tree() {
    return base_url('uploads/system/gallery/gallery_image_tree.jpg');
  }

  public function get_gallery_image_4() {
    return base_url('uploads/system/gallery/gallery_image_4.jpg');
  }

  public function get_gallery_image_5() {
    return base_url('uploads/system/gallery/gallery_image_5.jpg');
  }

  public function get_gallery_image_6() {
    return base_url('uploads/system/gallery/gallery_image_6.jpg');
  }

  public function get_gallery_image_7() {
    return base_url('uploads/system/gallery/gallery_image_7.jpg');
  }

  public function get_gallery_image_8() {
    return base_url('uploads/system/gallery/gallery_image_8.jpg');
  }

  public function get_gallery_image_9() {
    return base_url('uploads/system/gallery/gallery_image_9.jpg');
  }

  public function get_gallery_image_10() {
    return base_url('uploads/system/gallery/gallery_image_10.jpg');
  }
