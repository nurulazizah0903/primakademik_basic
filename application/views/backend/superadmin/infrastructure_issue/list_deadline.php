    ...
    <table id="basic-datatable" class="table table-striped dt-responsive nowrap" width="100%">
        <thead class="thead-dark">
            <tr>
                ...
                <th><?php echo get_phrase('lewat_hari'); ?></th>
        ...
        <tbody>
            <?php foreach ($infrastructure_issues as $infrastructure_issue):
                ...
                <td><?php echo $infrastructure_details['name']; ?></td>
                    ...
                    <td>
                        <?php 
                            $date1 =  date("d M Y", strtotime($infrastructure_issue['date']));
                            $date3 = date_create("$date1");
                            $date5 = date_create("$date");
                            $diff2 = date_diff($date3, $date5);
                        ?>
                        <?= $diff2->format('%a');?> <?php echo get_phrase('day'); ?>
                    </td>
                    <td>
                        <div class="dropdown text-center">
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                    ...
                                    <!-- item-->
                                    <a href="<?php echo route('infrastructure_issue/return/'.$infrastructure_issue['id']); ?>" class="dropdown-item"><?php echo get_phrase('infrastructure_already_return'); ?></a>
                                    <?php /* <a href="javascript:void(0);" class="dropdown-item" onclick="confirmModal('<?php echo route('infrastructure_issue/return/'.$infrastructure_issue['id']); ?>', showAllBookDeadline )"><?php echo get_phrase('infrastructure_already_return'); ?></a> */?>
                                    <!-- item-->
                                    <a href="javascript:void(0);" class="dropdown-item" onclick="confirmModal('<?php echo route('infrastructure_issue/delete/'.$infrastructure_issue['id']); ?>', showAllInfrastructureDeadline )"><?php echo get_phrase('delete'); ?></a>
                            </div>
                        </div>
                    </td>
                </tr>
...
<script>
    ...
    var form = $(this);
    ajaxSubmit(e, form, showAllInfrastructureDeadline);
    });
</script>