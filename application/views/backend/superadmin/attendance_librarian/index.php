...
<div class="row d-print-none">
  <div class="col-xl-12">
    <div class="card">
      <div class="card-body">
        <h4 class="page-title">
          <i class="mdi mdi-calendar-today title_icon"></i> <?php echo get_phrase('daily_attendance_librarian'); ?>
          <button type="button" class="btn btn-outline-primary btn-rounded alignToTitle" onclick="rightModal('<?php echo site_url('modal/popup/attendance_librarian/take_attendance_librarian'); ?>', '<?php echo get_phrase('take_attendance_librarian'); ?>')"> <i class="mdi mdi-plus"></i> <?php echo get_phrase('take_attendance_librarian'); ?></button>
        </h4>
      </div> <!-- end card body-->
    </div> <!-- end card -->
  </div><!-- end col-->
  ...
  <div class="col-12">
    <div class="card">
      <div class="row mt-3 d-print-none">
        ...
        <div class="col-md-2">
          <button class="btn btn-block btn-secondary" onclick="filter_attendance_librarian()" ><?php echo get_phrase('filter'); ?></button>
        </div>
      </div>
      <div class="card-body attendance_librarian_content">
        <div class="empty_box">
...
function filter_attendance_librarian(){
  var month = $('#month').val();
  var year = $('#year').val();
  if(month != "" && year != ""){
    getDailtyAttendanceLibrarian();
  }else{
    toastr.error('<?php echo get_phrase('please_select_in_all_fields !'); ?>');
  }
}

var getDailtyAttendanceLibrarian = function () {
  var month = $('#month').val();
  var year = $('#year').val();
  if(month != "" && year != ""){
    $.ajax({
      type: 'POST',
      url: '<?php echo route('attendance_librarian/filter') ?>',
      data: {month : month, year : year},
      success: function(response){
        $('.attendance_librarian_content').html(response);
