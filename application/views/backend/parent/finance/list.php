...
// if (isset($class_id) && isset($section_id)):
    $finances  = $this->db->get_where('finances', array('student_id' => $student_data['student_id'], 'session_id' => active_session()))->result_array();
    // var_dump($class_id);
    // die;
    if(count($finances) > 0):?>
            ...
            ?>
                <tr>
                    ...
                    <td>
                      ...
                      ?>
                        <a href="javascript:void(0);" class="btn btn-danger" onclick="rightModal('<?php echo site_url('modal/popup/finance/edit/'.$finance['id'])?>', '<?php echo get_phrase('upload_bukti'); ?>');"><?php echo get_phrase('upload'); ?></a>
                      <?php } ?>
                    </td>
                    <td>
