<?php
$section_data = $this->db->get_where('sections', array('id' => $param2))->row_array();
?>
<?php $school_id = school_id(); ?>
<center>
<form method="POST" class="d-block ajaxForm responsive_media_query" action="<?php echo route('attendance_routine/take_attendance_routine'); ?>" style="min-width: 300px; max-width: 600px;">
<input type="hidden" name="date" id="date_on_taking_attendance" value="<?= $param1?>">
<input type="hidden" name="class_id" id="class_id_on_taking_attendance" value="<?= $section_data['class_id']?>">
<input type="hidden" name="section_id" id="section_id_on_taking_attendance" value="<?= $param2?>">
<input type="hidden" name="subject_id" id="subject_id_on_taking_attendance" value="<?= $param3?>">
    ...
    <div class="row" id = "student_content" style="margin-left: 2px;">
        <div class="table-responsive-sm row col-md-12" style="padding-right: 0px;">
            <table class="table table-bordered table-centered mb-0">
                ...
                <tbody>
                    <?php $enrols = $this->db->get_where('enrols', array('class_id' => $section_data['class_id'], 'section_id' => $param2, 'school_id' => $school_id, 'session' => active_session()))->result_array(); ?>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="custom-control custom-radio">
                                    <?php $update_attendance = $this->db->get_where('daily_attendance_routines', array('timestamp' => strtotime(($param1)), 'class_id' => $section_data['class_id'], 'section_id' => $param2, 'subject_id' => $param3, 'session_id' => active_session(), 'school_id' => $school_id, 'student_id' => $enroll['student_id'])); ?>
