<script type="text/javascript">
	...
	var showExams = function () {
        var url = '<?php echo site_url('addons/exam/index/list'); ?>';
    ...
    function save_question_answer(question_id, exam_id, btn){
	    ...
	    }else if($(`input[name="question_answer_${question_id}"]:checked`).attr("type") == 'radio'){
	    	// var question_answer = $(`[name="question_answer_${question_id}"]`).val();
			var question_answer = $(`input[name="question_answer_${question_id}"]:checked`).val();
	    }else{
	    	var question_answer = $('#question_answer_'+question_id).val();
	    }
	    form_data.append('question_answer', question_answer);
	    $.ajax({
	        url: '<?= site_url('addons/exam/save_answers/'); ?>' + question_id + '/' + exam_id,
