<?php
    $school_id = school_id();
    $attendance_of_students = $this->db->get_where('daily_attendance_employee', array('id' => $param1))->result_array();
...
<div class="h-100">
        <div class="col-md-12">
            ...
            <form method="POST" class="d-block ajaxForm" action="<?php echo route('attendance_employee/confirm_employee/'.$param1); ?>" enctype="multipart/form-data">
            <table class="table table-centered mb-0">
                <tbody>
                    <tr>
                        <td style="font-weight: bold;"><?= get_phrase('keterangan_absen'); ?></td>
                        <td>
                            <?php  $status = $this->db->get_where('daily_attendance_employee', array('id' => $attendance_of_student['id']))->row('status');?>
                            <?php if($status == 1){ ?>
...
$(".ajaxForm").validate({}); // Jquery form validation initialization
    $(".ajaxForm").submit(function(e) {
        var form = $(this);
        ajaxSubmit(e, form, getDailtyAttendanceEmployee);
    });
</script>