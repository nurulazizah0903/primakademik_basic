...
<form method="POST" class="d-block ajaxForm responsive_media_query" action="<?php echo route('attendance_accountant/take_attendance_accountant'); ?>" style="min-width: 300px; max-width: 400px;">
    ...
    <div class='row'>
        <div class="form-group col-md-12" id="showStudentDiv">
            <a class="btn btn-block btn-secondary" onclick="getStudentList()" style="color: #fff;" disabled><?php echo get_phrase('show_accountant_list'); ?></a>
        </div>
    </div>
...
<script>
    ...
    $(".ajaxForm").submit(function(e) {
        var form = $(this);
        ajaxSubmit(e, form, getDailtyAttendanceAccountant);
    });
    ...
    function getStudentList() {
        ...
        if(date != ''){
            $.ajax({
                type : 'POST',
                url : '<?php echo route('attendance_accountant/accountant/'); ?>',
                data: {date : date},
                success : function(response) {
