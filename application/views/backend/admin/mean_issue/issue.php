      ...
      <div class="card-body">
        <div class="row mb-3">
          <div class="col-md-1 mb-1"></div>
            ...
            <div class="col-md-2">
                <button class="btn btn-block btn-secondary" onclick="filter_mean_issue()" ><?php echo get_phrase('filter'); ?></button>
            </div>
          </div>
...
function filter_mean_issue(){
    var mean_id = $('#mean_id').val();
    var date = $('#date').val();
    var tgl = $('#tgl').val();
    $.ajax({
        type : 'post',
        url: '<?php echo route('mean_issue/filter_issue/') ?>',
...
var showAllMeanIssues = function (){
  var url = '<?php echo route('mean_issue/list_issue'); ?>';
  $.ajax({
    type : 'GET',
    url: url,
    success : function(response) {
