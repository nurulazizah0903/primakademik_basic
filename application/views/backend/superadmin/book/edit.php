...
<form method="POST" class="d-block ajaxForm" action="<?php echo route('book/update/'.$param1); ?>" enctype="multipart/form-data">
    <div class="form-row">
        ...
        <label for="name"><?php echo get_phrase('jenis_buku'); ?></label>
            <select name="book_type_id" id="book_id_on_modal" class="form-control select2" data-toggle = "select2" required onchange="show_book_type_form(this.value)">
                <option value=""><?php echo get_phrase('select_book_type'); ?></option>
                <option value="0" <?php if ($book_details['book_type_id'] == "0"): ?> selected <?php endif; ?>><?php echo get_phrase('ebook'); ?></option>
                <?php
        ...
        <div class="form-group col-md-12" id="ebook_file">
            <label for="copies"><?php echo get_phrase('ebook'); ?></label>
            <input type="file" class="form-control" id="ebook" name = "ebook" min="0" required>
        </div>
...
if($('select').hasClass('select2') == true){
    ...
    function show_book_type_form(param) {
