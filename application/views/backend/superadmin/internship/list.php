...
<div class="table-responsive-sm">
    <table id="basic-datatable" class="table table-striped dt-responsive nowrap" width="100%">
      ...
      <tbody>
            ...
            <tr>
                <td>
                    <a href="javascript:void(0);" onclick="largeModal('<?php echo site_url('modal/popup/internship/detail_list/'.$data['id_internship'])?>', '<?php echo get_phrase('details_student_internship').' '.$data['company_name'] ?>')"><?= $data['company_name'] ?></a>
                </td>
                <td><?= date("d M Y", $data['start_date']) ?></td>
                <td><?= date("d M Y", $data['end_date']) ?></td>
                <td>
                    <a href="<?php echo route('internship/attendance/'.$data['id_internship']); ?>"><?= get_phrase('internship_attendance'); ?></a>
                </td>
                <td>
