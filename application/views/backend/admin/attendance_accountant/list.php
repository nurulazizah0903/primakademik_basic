    ...
    <table  class="table table-bordered table-sm">
        <thead class="thead-dark">
            <tr style="font-size: 12px;">
                <th width = "40px"><?php echo get_phrase('accountant'); ?> <i class="mdi mdi-arrow-down"></i> <?php echo get_phrase('date'); ?> <i class="mdi mdi-arrow-right"></i></th>
                <?php
        ...
        <tbody>
            <?php
            $accountant_id_count = 0;
            $active_sesstion = active_session();
            $this->db->order_by('user_id', 'asc');
            $attendance_of_accountants = $this->db->get_where('daily_attendance_accountants', array('school_id' => $school_id, 'session_id' => $active_sesstion))->result_array();
                foreach($attendance_of_accountants as $attendance_of_accountant){ ?>
                    <?php if(date('m', $attendance_date) == date('m', $attendance_of_accountant['timestamp'])): ?>
                        <?php if($accountant_id_count != $attendance_of_accountant['user_id']): ?>
                            <tr>
                                <td>
                                <?php echo $this->user_model->get_user_details($attendance_of_accountant['user_id'], 'name'); ?>
                                </td>
                                <?php for ($i = 1; $i <= $number_of_days; $i++): ?>
                                    ...
                                    <td class="text-center" <?= ($weekday == 7) ? "style=\"background-color: #ff000011;\"" : "" ?>>
                                        <?php $status = $this->db->get_where('daily_attendance_accountants', array('school_id' => $school_id, 'session_id' => $active_sesstion, 'user_id' => $attendance_of_accountant['user_id'], 'timestamp' => $timestamp))->row('status'); 
                                        // var_dump($status);
                                        ?>
                                        <?php if($status == 1){ ?>
                        ...
                        <?php endif; ?>
                        <?php $accountant_id_count = $attendance_of_accountant['user_id']; ?>
                    <?php endif; ?>                 
                <?php } ?>
        </tbody>
    </table>
