<?php
  ...
  $finances = $this->db->get_where('finances', array('invoices_id' => $invoice_id))->result_array();  
  // print_r($finances); die;
  $savings = $this->db->get_where('savings', array('invoices_id' => $invoice_id))->row_array(); 
          ...
          <div class="col-sm-6">
            <div class="float-left mt-1">
              ...
              <address>
                ...
                <?php elseif (strtolower($invoice_details['status']) == 'not_yet_paid_off'): ?>
                  <span class="badge badge-primary-lighten"><?php echo get_phrase('not_yet_paid_off'); ?></span></p>
                <?php else: ?>
        ...
        <div class="row">
          ...
          <div class="row">
            ...
            <div class="col-sm-6">
              <div class="float-right mt-3 mt-sm-0">
                ...
                <p><b>Total Tagihan &nbsp;</b> <span class="float-right"><?php echo currency( number_format($invoice_details['total_amount'])); ?></span></p>
                <?PHP 
                  $total_bayar = 0; $no = 1;
                  foreach ($finances as $finance): 
                    $total_bayar = $total_bayar + $finance['total'];
                ?> 
                  <p><b>Pembayaran <?php echo $no ." : ".date('M/d/Y', $finance['created_at']); ?>  &nbsp;</b> <span class="float-right"> <?php echo currency( number_format($finance['total'],0,",",".")); ?></span></p> 
                <?php $no++; endforeach; ?> 
                
                <?php 
                  $saldo = $invoice_details['total_amount'] - $total_bayar;
                  if($saldo > 0){
                ?>
                <hr></hr>
                <p><b>Belum Dibayar &nbsp;</b> <span class="float-right"><?php echo currency( number_format($saldo,0,",",".")); ?></span></p>
                <?php }
                  if(isset($savings) && empty($savings['status'])){ 
                ?>
                <hr></hr>
                <p><b>Uang Titipan &nbsp;</b> <span class="float-right"><?php echo currency( number_format($savings['total'],0,",",".")); ?></span></p>
                <?PHP } ?>
              </div>
          ...
          <div class="d-print-none mt-4">
            <div class="text-right">
              <a href='<?php echo route('invoice'); ?>' class="btn btn-success"><i class="mdi mdi-backspace"></i> <?php echo get_phrase('back'); ?></a>
