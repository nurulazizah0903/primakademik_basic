...
<div class="row d-print-none">
  <div class="col-xl-12">
    <div class="card">
      <div class="card-body">
        <h4 class="page-title">
          <i class="mdi mdi-calendar-today title_icon"></i> <?php echo get_phrase('daily_attendance_teacher'); ?>
          <button type="button" class="btn btn-outline-primary btn-rounded alignToTitle" onclick="showAjaxModal('<?php echo site_url('modal/popup/attendance_teacher/take_attendance_teacher'); ?>', '<?php echo get_phrase('take_attendance_teacher'); ?>')"> <i class="mdi mdi-plus"></i> <?php echo get_phrase('take_attendance_teacher'); ?></button>
        </h4>
      </div> <!-- end card body-->
    </div> <!-- end card -->
  </div><!-- end col-->
...
var getDailtyAttendance = function () {
  ...
  if(month != "" && year != ""){
    $.ajax({
      type: 'POST',
      url: '<?php echo route('attendance_teacher/filter') ?>',
