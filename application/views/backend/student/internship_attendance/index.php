<?php
    ...
    $internship_id = $this->db->get_where('internship_student', array('student_id' => $student_data['id']))->row('internship_id');
?>
<!--title-->
...
var getDailtyAttendance = function () {
  ...
  var month = $('#month').val();
  var student_id = $('#student_id').val();
  if(month != "" && internship_id != "" && student_id != ""){
    $.ajax({
      type: 'POST',
      url: '<?php echo route('internship_attendance/attendance_filter') ?>',
      data: {month: month, internship_id: internship_id, student_id: student_id},
      success: function(response){
