...
<div class="row d-print-none">
  <div class="col-xl-12">
    <div class="card">
      <div class="card-body">
        <h4 class="page-title">
          <i class="mdi mdi-calendar-today title_icon"></i> <?php echo get_phrase('daily_attendance_accountant'); ?>
          <button type="button" class="btn btn-outline-primary btn-rounded alignToTitle" onclick="rightModal('<?php echo site_url('modal/popup/attendance_accountant/take_attendance_accountant'); ?>', '<?php echo get_phrase('take_attendance_accountant'); ?>')"> <i class="mdi mdi-plus"></i> <?php echo get_phrase('take_attendance_accountant'); ?></button>
        </h4>
      </div> <!-- end card body-->
    </div> <!-- end card -->
  </div><!-- end col-->
  ...
  <div class="col-12">
    <div class="card">
      <div class="row mt-3 d-print-none">
        ...
        <div class="col-md-2">
          <button class="btn btn-block btn-secondary" onclick="filter_attendance_accountant()" ><?php echo get_phrase('filter'); ?></button>
        </div>
      </div>
      <div class="card-body attendance_accountant_content">
        <div class="empty_box">
...
function filter_attendance(){
  ...
  var year = $('#year').val();
  if(month != "" && year != ""){
    getDailtyAttendanceAccountant();
  }else{
    toastr.error('<?php echo get_phrase('please_select_in_all_fields !'); ?>');
  }
}

var getDailtyAttendanceAccountant = function () {
  var month = $('#month').val();
  var year = $('#year').val();
  if(month != "" && year != ""){
    $.ajax({
      type: 'POST',
      url: '<?php echo route('attendance_accountant/filter') ?>',
      data: {month : month, year : year},
      success: function(response){
        $('.attendance_accountant_content').html(response);
