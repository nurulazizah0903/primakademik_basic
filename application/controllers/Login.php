...
class Login extends CI_Controller {
	...
	public function index(){
		if($this->session->userdata('superadmin_login') ==  true){
			...
			redirect(route('dashboard'), 'refresh');
		}elseif($this->session->userdata('apd_login') ==  true){
			redirect(route('dashboard'), 'refresh');
		}else{
	...
	public function validate_login(){
		...
		if ($query->num_rows() > 0) {
			...
			if ($row->role == 'superadmin' || $row->role == 'accountant' || $row->role == 'admin') {
				$postdata = [
					'params' => [
						'email' => $email,
						'password' => $password,
				...
				$loginOdoo = $this->http->post(config_item('odoo_url').'/web/tes', json_encode($postdata));
			}
			...
			elseif($row->role == 'admin_ppdb'){
				$this->session->set_userdata('apd_login', true);
				$this->session->set_userdata('user_id', $row->id);
				$this->session->set_userdata('school_id', $row->school_id);
				$this->session->set_userdata('user_name', $row->name);
				$this->session->set_userdata('user_type', 'apd');
				$this->session->set_flashdata('flash_message', get_phrase('welcome_back'));
				redirect(site_url('apd/registration/create'), 'refresh');
			}
		}else{
	...
	public function retrieve_password() {
		...
		if ($query->num_rows() > 0) {
			...
			$history_data['ket'] = 'Update data users '.$query['id'].'';
