...
<center>
<form method="POST" class="d-block ajaxForm responsive_media_query" action="<?php echo route('attendance_routine/take_attendance_routine'); ?>" style="min-width: 300px; max-width: 400px;">
    ...
    <div class="form-group row">
        <div class="col-md-12" id = "section_content_2">
            ...
            <select name="section_id" id="section_id_on_taking_attendance" class="form-control select2" data-toggle="select2" required onchange="classWiseSubjectOnTakingAttendance(this.value), sectionWiseClassroomsOnCreate(this.value)">
    ...
    <div class="form-group row">
        <div class="col-md-12">
            <label for="subject_id_on_taking_attendance"><?php echo get_phrase('subject'); ?></label>
            <select name="subject_id" id="subject_id_on_taking_attendance" class="form-control select2" data-toggle="select2" required >
...
<script>
    ...
    $('document').ready(function(){
        initSelect2(['#class_id_on_taking_attendance', '#room_id_on_taking_attendance', '#section_id_on_taking_attendance', '#subject_id_on_taking_attendance']);
        ...
        $('#subject_id_on_taking_attendance').change(function(){
            $('#showStudentDiv').show();
            $('#updateAttendanceDiv').hide();
            $('#student_content').hide();
        });
    ...
    function classWiseSectionOnTakingAttendance(classId) {
        $.ajax({
            ...
            classWiseSubjectOnTakingAttendance(classId);
            }
        });
    }

    function classWiseSubjectOnTakingAttendance(classId) {
    $.ajax({
        url: "<?php echo route('class_wise_subject/'); ?>"+classId,
        success: function(response){
            $('#subject_id_on_taking_attendance').html(response);
        }
    });
    }
    ...
    function getStudentList() {
        ...
        var subject_id = $('#subject_id_on_taking_attendance').val();
        var room_id = $('#room_id_on_taking_attendance').val();

        if(date != '' && class_id != '' && section_id != '' && room_id != '' && subject_id != ''){
            $.ajax({
                type : 'POST',
                url : '<?php echo route('attendance_routine/student/'); ?>',
                data: {date : date, class_id : class_id, section_id : section_id, room_id : room_id, subject_id : subject_id},
                success : function(response) {
