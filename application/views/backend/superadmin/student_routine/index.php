...
<div class="row ">
  <div class="col-xl-12 d-print-none">
    <div class="card">
      <div class="card-body">
        <h4 class="page-title">
		    <i class="mdi mdi-calendar-today title_icon"></i> <?php echo get_phrase('manage_student_routine'); ?>
        </h4>
      </div> <!-- end card body-->
    </div> <!-- end card -->
  </div><!-- end col-->
...
$('document').ready(function(){
    initSelect2(['#student_id','#section_id','#room_id','#class_id']);
});
...
function filter_student(){
    ...
    $.ajax({
        type : 'post',
        url: '<?php echo route('student_routine/filter/') ?>',
...
var showAllStudents = function() {
        $.ajax({
            url: '<?php echo route('student_routine/list/') ?>',
            success: function(response){
                $('.student_info_content').html(response);
            }
        });
