...
//-----------------TEACHER------------------//
	...
	public function questions($param1 = "", $param2 = "", $param3 = ""){
		...
		if($param1 == 'add_question'){
			$response = $this->assignment_model->add_question($param2);
			echo $response;
			$this->session->set_flashdata('flash_message', get_phrase('question_added_successfully'));
			redirect(site_url('addons/assignment/questions/'.$param2), 'refresh');
		}

		if($param1 == 'add_question_bank'){
			$response = $this->assignment_model->add_question_bank($param2);
			echo $response;
		}

		if($param1 == 'take_question'){
			$response = $this->assignment_model->take_question($param2,$param3);
			$questions = $this->db->get_where('assignment_questions', array('id' => $param3))->row_array();
			echo $response;
			$this->session->set_flashdata('flash_message', get_phrase('question_updated_successfully'));
			redirect(site_url('addons/assignment/questions/'.$questions['assignment_id']), 'refresh');
		}
		...
		if($param1 == 'numbering'){
			...
			$history_data['ket'] = 'Update data assignment questions '.$value.'';
		...
		if($param1 == 'excel_question'){
			$response = $this->assignment_model->excel_question($param2);
			echo $response;
		}		
	...
	public function submit_mark($assignment_id = "", $student_id = ""){
		if($this->session->userdata('teacher_login') != 1){
			...
			$this->assignment_model->submit_mark($assignment_id, $student_id);
	...
	public function update_remark($assignment_id = "", $student_id = ""){
		...
		if($query->num_rows() > 0){
			...
			$history_data['ket'] = 'Update data assignment remarks '.$assignment_id.'';
			$history_data['id_user'] = $this->session->set_userdata('user_id');
			$this->db->insert('history', $history_data);
			...
			$history_data['ket'] = 'Mengisi data assignment_remarks';
			$history_data['id_user'] = $this->session->set_userdata('user_id');
			$this->db->insert('history', $history_data);
		}
...
//-----------------STUDENT------------------//
	public function my_active_assignment($param1 = "", $param2 = "", $param3 = ""){
		...
		if(empty($param1)){
			...
			$this->load->view('backend/index', $page_data);
			// $this->load->view('backend/student/assignment/index', $page_data);
		}
	}
	...
	public function assignment_questions($param1 = "", $param2 = ""){
		...
		$this->load->view('backend/index', $page_data);
		// $this->load->view('backend/student/assignment/questions', $page_data);
	...
	public function my_assignment_result($assignment_id = ""){
		...
		$page_data['page_title'] = 'assignment_result';
		$this->load->view('backend/index', $page_data);
		// $this->load->view('backend/student/assignment/assignment_result', $page_data);
	}
//-----------------STUDENT END------------------//
