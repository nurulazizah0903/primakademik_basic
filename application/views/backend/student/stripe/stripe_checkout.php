...
<!--required for getting the stripe token-->
        <?php
            $stripe_test_mode = get_payment_settings('stripe_mode');
            if ($stripe_test_mode == 'on') {
                $public_key = get_payment_settings('stripe_test_public_key');
                $private_key = get_payment_settings('stripe_test_secret_key');
            } else {
                $public_key = get_payment_settings('stripe_live_public_key');
                $private_key = get_payment_settings('stripe_live_secret_key');
            }
        ?>
...
<!--required for getting the stripe token-->
        ...
        <img src="<?php echo $this->settings_model->get_logo_light(); ?>" width="15%;"
            ...
            <form method="post"
              ...
              <button type="submit">
                  <?php echo get_phrase('pay');?> <?php echo $amount_to_pay.' '.get_payment_settings('stripe_currency');?>
              </button>
              <div class="outcome">
        ...
        <script type="text/javascript">
            get_stripe_currency('<?php echo get_payment_settings('stripe_currency'); ?>');
        </script>
