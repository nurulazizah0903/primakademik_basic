        ...
        <thead class="thead-dark">
            <tr style="font-size: 10px;">
                ...
                <?php
                    ...
                    ?>
                    <th><a type="button" onclick="largeModal('<?php echo site_url('modal/popup/attendance/modal_add/'.$tanggal.'/'.$class_id.'/'.$section_id)?>', '<?= get_phrase('take_attendance'); ?>')" data-toggle="tooltip" data-placement="top" title=""><?php echo $i; ?></a></th>
                <?php endfor; ?>
            </tr>
        </thead>
        <tbody>
                            ...
                            <tr>
                                ...
                                <?php for ($i = 1; $i <= $number_of_days; $i++): ?>
                                    ...
                                    <td class="text-center" <?= ($weekday == 7) ? "style=\"background-color: #ff000011;\"" : "" ?>>
                                        ...
                                        <?php if($status == 1){ ?>
                                            <a onclick="largeModal('<?php echo site_url('modal/popup/attendance/confirm/'.$attendance['id'])?>', '<?= get_phrase('attendance_confirm'); ?>')" data-toggle="tooltip" data-placement="top" title=""><i class="mdi mdi-circle text-success" title="Masuk"></i></a>
                                        <?php }elseif($status === "2"){ ?>
                                            <a onclick="largeModal('<?php echo site_url('modal/popup/attendance/confirm/'.$attendance['id'])?>', '<?= get_phrase('attendance_confirm'); ?>')" data-toggle="tooltip" data-placement="top" title=""><i class="mdi mdi-circle text-warning" title="Izin"></i></a>
                                        <?php }elseif($status === "3"){ ?>
                                            <a onclick="largeModal('<?php echo site_url('modal/popup/attendance/confirm/'.$attendance['id'])?>', '<?= get_phrase('attendance_confirm'); ?>')" data-toggle="tooltip" data-placement="top" title=""><i class="mdi mdi-circle text-primary" title="Sakit"></i></a>
                                        <?php }elseif($status === "0"){ ?>
                                            <a onclick="largeModal('<?php echo site_url('modal/popup/attendance/confirm/'.$attendance['id'])?>', '<?= get_phrase('attendance_confirm'); ?>')" data-toggle="tooltip" data-placement="top" title=""><i class="mdi mdi-circle text-danger" title="Absen"></i></a>
                                        <?php }elseif($status === "-1"){ ?>
                                            <a onclick="largeModal('<?php echo site_url('modal/popup/attendance/confirm/'.$attendance['id'])?>', '<?= get_phrase('attendance_confirm'); ?>')" data-toggle="tooltip" data-placement="top" title=""><i class="mdi mdi-circle text-secondary"></i></a>
                                        <?php } ?>
                                    </td>
                                <?php endfor; ?>
                            </tr>
